/*
This file defines methods for operations we wish to perform
on solenoids. 
...
void SolenoidControl::toggleSolenoid(size_t index){
...
void SolenoidControl::activateSolenoid(size_t index) {
    if (index >= state.solenoid_count()) return;
...
void SolenoidControl::deactivateSolenoid(size_t index) {
    if (index >= state.solenoid_count()) return;
    
    state.setActive(index, false);
    sendCommand(CMD_SOLENOID, static_cast<unsigned char>(index), 0x00, 0x00);
}


void SolenoidControl::programSolenoid(size_t index, int openTime, int closeTime) {
    if (index >= state.solenoid_count()) return;
...
void SolenoidControl::clearSolenoid(size_t index) {
    if (index >= state.solenoid_count()) return;
...
void SolenoidControl::setPositivePressure(int percentage) {
    if (percentage < 0 || percentage > 100) return;
    
    sendCommand(CMD_PRESSURE, 0x01, static_cast<unsigned char>(percentage), 0x00);
}

void SolenoidControl::setVacuum(int percentage) {
    if (percentage < 0 || percentage > 100) return;
